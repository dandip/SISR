...
class DSRRNN(nn.Module):
    def __init__(self, operators, hidden_size, device, left_separability,
                 right_separability, type='rnn', num_layers=1,
                 dropout=0.0, left_min_length=2,
                 left_max_length=6, right_min_length=2, right_max_length=6,
                 mutation_rate=0.0):
        ...
        # Details related to separability
        ...
        self.left_separability = left_separability
        self.right_separability = right_separability
    ...
    def sample_separable_sequence(self, n, left_symmetric=False, right_symmetric=False):
        sequences = []
        sequences_tensors = []
        sequence_lengths = []
        entropies = []
        log_probs = []
        # Sample left terms
        for i in range(len(self.operators.left_variables)):
            if (i==0 or not left_symmetric):
                term_sequences, term_sequence_lengths, term_entropies, term_log_probs = self.sample_sequence('left', i, n)
                sequences.append(term_sequences.long().tolist())
                sequences_tensors.append(term_sequences.long())
                sequence_lengths.append(term_sequence_lengths.long().tolist())
                entropies.append(term_entropies)
                log_probs.append(term_log_probs)
            else:
                working_sequence = sequences_tensors[-1]
                # Map working sequence to variables in the second term
                working_sequence += torch.isin(working_sequence, self.operators.variable_tensor).long() * len(self.operators.left_variables[-1])
                sequences.append(working_sequence.long().tolist())
                sequences_tensors.append(working_sequence)
                sequence_lengths.append(sequence_lengths[-1])
        for i in range(len(self.operators.right_variables)):
            if (i==0 or not right_symmetric):
                term_sequences, term_sequence_lengths, term_entropies, term_log_probs = self.sample_sequence('right', i, n)
                sequences.append(term_sequences.long().tolist())
                sequences_tensors.append(term_sequences.long())
                sequence_lengths.append(term_sequence_lengths.long().tolist())
                entropies.append(term_entropies)
                ...
                working_sequence = sequences_tensors[-1]
                # Map working sequence to variables in the second term
                working_sequence += torch.isin(working_sequence, self.operators.variable_tensor).long() * len(self.operators.right_variables[-1])
                sequences.append(working_sequence.long().tolist())
                sequences_tensors.append(working_sequence)
                sequence_lengths.append(sequence_lengths[-1])
        entropies = torch.stack(entropies).sum(axis=0)
        log_probs = torch.stack(log_probs).sum(axis=0)
        sequences = [list(x) for x in list(zip(*sequences))]
        sequence_lengths = [list(x) for x in list(zip(*sequence_lengths))]
        return sequences, sequence_lengths, entropies, log_probs

    def sample_sequence(self, side, term, n):
        ...
        # Used to track if we're inside of a trigonometric expression (prevent nesting)
        trig_counter = torch.zeros(n)
        inside_trig = torch.zeros(n, dtype=torch.bool)
        ...
        while(sequence_mask.all(dim=1).any()):
            ...
            output = self.apply_constraints(output, counters, lengths, sequences, inside_trig, side, term)

            # All constraints have been applied: normalize the distribution
            ...
            # Random mutation; chance to override with randomly sampled token
            # from output
            if (self.mutation_rate > 0):
                # Create uniform distribution within determined constraints
                uniform_output = ((output > 0).long())
                uniform_output = uniform_output / torch.sum(uniform_output, axis=1)[:, None]
                uniform_distribution = torch.distributions.Categorical(uniform_output)
                uniform_token = dist.sample()
                # Roll whether or not to mutate and update token accordingly. Note
                # that we will still use probability from the original distribution
                # for computing our loss, rather than the uniform distribution.
                mutation_mask = torch.rand(token.shape) <= self.mutation_rate
                token = token * (~(mutation_mask)).long() + uniform_token * mutation_mask.long()

            # Add sampled tokens to sequences
            sequences = torch.cat((sequences, token[:, None]), axis=1)
            lengths += 1 * sequence_mask[:, -1].long()
            ...
            # Update whether or not we're inside of a trig function
            if (self.operators.contains_trig()):
                hit_trig_counter = (counters == trig_counter)
                inside_trig = torch.logical_and(~hit_trig_counter, inside_trig)
                sampled_trig = torch.isin(token, self.operators.trig_indices)
                trig_counter = sampled_trig * (counters-1)
                inside_trig += sampled_trig
    ...
    def apply_constraints(self, output, counters, lengths, sequences, inside_trig, side, term):
        ...
        # Set the appropriate minimum and max based on side and term
        if (side=='left'):
            min_length = self.left_min_length[term]
            max_length = self.left_max_length[term]
            allowable_terms = self.operators.left_mask[term]
        else:
            min_length = self.right_min_length[term]
            max_length = self.right_max_length[term]
            allowable_terms = self.operators.right_mask[term]

        # ~ Check that minimum length will be met ~
        min_boolean_mask = (counters + lengths >= torch.ones(counters.shape) * min_length).long()[:, None]
        ...
        max_boolean_mask = (counters + lengths <= torch.ones(counters.shape) * (max_length - 2)).long()[:, None]
        ...
        # Prevent us from sampling a trigonometric operator if we're already inside of one
        if (self.operators.contains_trig()):
            trig_mask = self.operators.trig_mask.repeat(counters.shape[0], 1) * inside_trig[:, None]
            trig_mask += torch.ones(counters.shape[0], len(self.operators)) * ((~inside_trig).long())[:, None]
            output = torch.minimum(output, trig_mask)

        # Get rid of variables that aren't allowed for the current side/term
        separability_mask = (allowable_terms.repeat(counters.shape[0], 1)).long()
        output = torch.minimum(output, separability_mask)
